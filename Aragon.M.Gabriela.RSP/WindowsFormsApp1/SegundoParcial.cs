...
namespace SP
{
    ...
    public partial class SegundoParcial: Form
    {
        ...
        {
            MessageBox.Show("Aragon Gabriela 2°A");
        }
        ...
        private static string ObtenerListadoFrutas()
        {
            ...
            //SqlConnection connection = connection = new SqlConnection(SP.Properties.Settings.Default.Conexion);
            //SqlCommand command;
            ...
            //{
            //    connection.Open();
            //    command = new SqlCommand("SELECT [id],[descripcion],[precio] FROM frutas", connection);
        ...
        private static bool AgregarFrutas(SegundoParcial frm)
        {
            ...
            command.CommandText = String.Format("INSERT INTO sp_lab_II.dbo.frutas (nombre, peso, precio) values ('{0}', {1}, {2})", sp._manzana.Nombre, sp._manzana.Peso, (double)60);
            try
            {
                conexion.Open();
            ...
            command.CommandText = String.Format("\nINSERT INTO sp_lab_II.dbo.frutas (nombre, peso, precio) values ('{0}', {1}, {2})", sp._durazno.Nombre, sp._durazno.Peso, (double)65);
            try
            {
                conexion.Open();
                command.ExecuteNonQuery();
            ...
            command.CommandText = String.Format("\nINSERT INTO sp_lab_II.dbo.frutas (nombre, peso, precio) values ('{0}', {1}, {2})", sp._banana.Nombre, sp._banana.Peso, (double)45);
